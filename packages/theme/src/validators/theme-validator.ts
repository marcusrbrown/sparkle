 ...
 */
function validateStructure(theme: ThemeConfig, errors: ValidationError[], _warnings: ValidationError[]): void {
 ...
 */
function isSemanticColor(colorKey: string): boolean {
 ...
 */
function isValidCSSUnit(value: string | number): boolean {
 ...
 */
function isValidFontWeight(weight: string | number): boolean {
 ...
 */
function isValidBoxShadow(shadow: string): boolean {
...
function validateColors(
  ...
  for (const colorKey of colorKeys) {
    ...
    for (const scaleKey of scaleKeys) {
      ...
      if (typeof colorValue === 'string' && options.validateColorContrast && isSemanticColor(colorKey)) {
        const contrast = calculateContrastRatio(colorValue, '#ffffff')
 ...
 */
function validateTypography(typography: ThemeConfig['typography']): {
  errors: ValidationError[]
  warnings: ValidationError[]
  checkedCount: number
} {
 ...
 */
function validateShadows(shadows: ThemeConfig['shadows']): {
  errors: ValidationError[]
  warnings: ValidationError[]
  checkedCount: number
} {
 ...
 */
function validateBorderRadius(borderRadius: ThemeConfig['borderRadius']): {
  errors: ValidationError[]
  warnings: ValidationError[]
  checkedCount: number
} {
  ...
  for (const [key, value] of borderRadiusEntries) {
    ...
    if (!isValidCSSUnit(value) && value !== 'full') {
      errors.push({
 ...
 */
function validateAnimation(animation: ThemeConfig['animation']): {
  errors: ValidationError[]
  warnings: ValidationError[]
  checkedCount: number
} {
...
export function validateTheme(theme: ThemeConfig, options: ValidationOptions = {}): ValidationResult {
  ...
  validateStructure(theme, errors, warnings)
  checkedProperties += 6 // Main theme properties

  // Validate colors
  const colorResult = validateColors(theme.colors, opts)
  ...
  // Validate animation
  const animationResult = validateAnimation(theme.animation)
