...
/**
 * Shell configuration manager implementation.
 */
export class ShellConfigManager implements ConfigManager {
  ...
  constructor(options: ConfigManagerOptions = {}) {
    ...
    this.storage = options.storage ?? new LocalStorageConfigStorage(options.storageKey)
  ...
  readonly set = async (path: string, value: unknown): Promise<void> => {
    ...
    try {
      ...
      this.emitChange({
        ...
        newValue: value,
        isBulkUpdate: false,
        timestamp: this._config.lastUpdated,
      })
  ...
  readonly update = async (updates: Record<string, unknown>): Promise<void> => {
  ...
  readonly reset = async (section?: keyof ShellConfig): Promise<void> => {
    try {
      const oldConfig = structuredClone(this._config)

      if (section) {
        // Reset specific section
        this._config[section] = structuredClone(DEFAULT_CONFIG[section]) as never
      } else {
      ...
      this.emitChange({
        ...
        newValue: section ? this._config[section] : this._config,
        isBulkUpdate: false,
        timestamp: this._config.lastUpdated,
      })
  ...
  readonly validate = (): ConfigValidationResult => {
    ...
    try {
      ...
      if (!['dark', 'light', 'solarized-dark', 'solarized-light', 'custom'].includes(this._config.appearance.theme)) {
        errors.push(`Invalid theme: ${this._config.appearance.theme}`)
      }

      if (![10, 11, 12, 13, 14, 15, 16, 18, 20, 22, 24].includes(this._config.appearance.fontSize)) {
        errors.push(`Invalid font size: ${this._config.appearance.fontSize}`)
      }

      if (this._config.appearance.scrollback < 100) {
        warnings.push('Scrollback buffer is very small, consider increasing for better history')
      }

      // Validate behavior config
      if (!['default', 'minimal', 'verbose', 'git-aware', 'custom'].includes(this._config.behavior.prompt.style)) {
        errors.push(`Invalid prompt style: ${this._config.behavior.prompt.style}`)
      }

      if (this._config.behavior.completion.maxSuggestions < 1) {
        errors.push('Max suggestions must be at least 1')
      }

      if (this._config.behavior.history.maxHistorySize && this._config.behavior.history.maxHistorySize < 10) {
        warnings.push('History size is very small, consider increasing for better history management')
      }

      // Validate security config
      if (this._config.security.maxWasmSize < 1 || this._config.security.maxWasmSize > 100) {
  ...
  readonly export = (): ConfigExport => {
    return {
      config: structuredClone(this._config),
      metadata: {
        exportedAt: new Date(),
        shellVersion: this._config.version,
        userAgent: globalThis.navigator?.userAgent ?? 'Unknown',
      },
    }
  }

  readonly import = async (configExport: ConfigExport): Promise<void> => {
    try {
      const validation = this.validateImport(configExport)
      if (!validation.isValid) {
        throw new Error(`Invalid configuration import: ${validation.errors.join(', ')}`)
      }

      const oldConfig = structuredClone(this._config)
      this._config = structuredClone(configExport.config)
      this._config.lastUpdated = new Date()
  ...
  readonly load = async (): Promise<void> => {
    try {
      ...
      if (stored) {
        ...
        this._config = this.mergeWithDefaults(stored)
   ...
   */
  private validateImport(configExport: ConfigExport): ConfigValidationResult {
    const errors: string[] = []
   ...
   */
  private mergeWithDefaults(stored: ShellConfig): ShellConfig {
    return {
      ...DEFAULT_CONFIG,
      ...stored,
      appearance: {
...
export function createConfigManager(options?: ConfigManagerOptions): ConfigManager {
  return new ShellConfigManager(options)
}
