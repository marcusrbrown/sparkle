...
- Once preflight passes, three passes populate the staged Deciduous instance, in order so later passes can link to nodes earlier passes created:
  ...
  3. **PR-body pass**: the eligible PR set is **merged PRs within the last 12 calendar months, ending at the build's captured snapshot timestamp** (inclusive of both the cutoff date — 12 calendar months before the snapshot — and the snapshot timestamp itself as the upper bound), **union** any older merged PR explicitly referenced by a selected in-scope `.ai/` artifact (de-duplicated against the calendar-window set so a PR matching both criteria is processed once). No open/unmerged PRs, no PRs merged after the captured snapshot timestamp. The cutoff date and the PR-list fetch timestamp are recorded in the staging directory's local provenance (see Stage 1 provenance above) alongside the other snapshot metadata. For each eligible PR, the script first looks for a matching action node to link against, THEN creates the decision node with `deciduous add decision "<PR title>" --description="<normalized summary>" --files "<file list from PR>" --commit <merge SHA> -c <confidence>` (`=`-bound value, never a bare positional, so a normalized summary beginning with `-` cannot collide with argv parsing) — no matching action found: creates at confidence 70 with a warning logged, no `deciduous link` is attempted; a matching action found: creates at confidence 75, then calls `deciduous link` to it — since v0.17.1's CLI has no confidence-edit command, a **failed** link attempt at this point cannot retroactively lower the already-created decision's confidence, so the whole build fails instead, preserving the staged output for diagnosis (no accept/promote proceeds). `--description` is used for the normalized summary rather than `-p`/`--prompt` — the summary's role is a description, and no long-form prompt text is needed. `--description` never carries the raw PR body — the script builds a normalized summary first (see Secrets/PII safety); the raw body is read only in memory to produce it. The normalized summary is bounded to 2,000 Unicode characters and deterministically strips common boilerplate/release-note scaffolding (checklists, badge/banner lines, changelog auto-headers) before clipping — a fixed rule set, not a model call.
...
- Validate three artifacts against their own pinned schemas, separately — the committed record JSON (`.deciduous/sync/{nodes,edges,themes,tags}/*.json`), `docs/public/graph-data.json`, and `docs/public/git-history.json`. `graph-data.json`'s node/edge field allowlist (Operational Notes) does not apply to `git-history.json`'s schema — they are different shapes; if `git-history.json`'s exact schema hasn't yet been captured with evidence, that capture is a small implementation-time task for whoever builds this script, not something to invent here. `git-history.json` carries the commits `deciduous sync` selects as referenced by the graph's own records, not unconditionally every commit in the pinned snapshot's full history — the Git log pass (above) still walks and considers the full input; deciduous's own commit-selection logic for this export is what determines the final count (one observed run: 1,462 of ~2,285 commits — a historical example, not a hardcoded policy).
- **Secret-scrub, fail-closed, full-payload**: scans the complete record payload (every field, not only descriptions) and the complete serialized content of both export files — including the raw commit-message bodies `git-history.json` carries — for known-secret patterns (tokens, passwords, API keys, bearer/auth headers, credentialed URLs, AWS-style access keys, etc.). This runs once before any `deciduous add`/`doc attach` call (on the normalized inputs about to be written) and again over the fully staged output (records + both exports) before Stage 3 review. On a match anywhere in retained metadata, a description, a raw attachment, the record JSON, or either export, the build aborts and reports only the file path and which rule matched — never the matched value; that hard-fail behavior is unchanged. A match that occurs _only_ within content the normalizer already, deterministically omits or clips — never reaching the retained description, the records, or either export — is instead logged as a non-blocking warning (PR/artifact identifier, rule name, a hash of the omitted content — never the value) inside the same digest-bound review gate as every other build warning; it does not require a separate per-warning acknowledgment step. All build warnings, including these, are persisted to the staging directory's provenance before either export is written; a staged build with no persisted warnings record is treated as invalid output, never as "zero warnings occurred." A caught hard-fail secret blocks promotion pending explicit human review; there is no automatic redact-and-continue path.
...
**Secrets/PII safety** (build and validate stages): the input normalizer scrubs known-secret patterns before any `deciduous add`/`doc attach` call, per the fail-closed rule in Stage 2. `-d`/`--description` values are always the normalizer's output, never a raw PR body. All shell-out invocations (`deciduous`, `gh`) pass argv arrays with `shell: false` — never string-interpolated into a shell command — because commit summaries, PR titles, and PR bodies are external content the script does not otherwise control; free-text option values use the `--flag=value` bound form (see the PR-body and triage passes above) so a value beginning with `-` is never misread as a flag. Per-invocation timeouts and output-buffer limits are otherwise unchanged (15s, 2MB) except `deciduous sync`, which gets a longer 60s budget — sized against a measured ~22s real run against 1,462 referenced commits, not an arbitrary increase.
