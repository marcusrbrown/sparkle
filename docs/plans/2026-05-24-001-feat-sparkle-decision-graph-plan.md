...
- Grounded-chain check: at least one complete, evidence-backed `goal → option → decision → action → outcome` chain exists in the staged graph overall; every one of the 16 required triage artifacts is represented by at least one node; every generated node/edge traces back to a real source input (a commit SHA, a PR number, or a triage-report path). The script never manufactures a node or edge to hit a count target.
